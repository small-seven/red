...
def evaluate_attack(model, test_loader, atk, atk_name):
    ...
    logger.info(f'Attack_type: [{atk_name}],\t'
                ...
                f'acc: {float(meter.avg("test_acc")): .4f},\t'
...
def main(args):
    ...
    autoattack_loader = get_loaders(args)
    ...
    if f'{args.model}{args.depth}' == 'resnet18':
        if args.train_mode == "vanilla":
            model = ResNet18ForCIFAR(pretrained=False)
            load_name += f"_{args.train_mode}.pth"
            ckpt = torch.load(os.path.join(args.load_dir, load_name))
            model.load_state_dict(ckpt['model'])
            model = torch.nn.DataParallel(model).to(args.device)
            model.eval()
        else:
            if args.train_mode in ["red", "hyper"]:
                load_name += f"_{args.train_mode}_n{args.num_models}_s1"
            else:
                load_name += f"_{args.train_mode}_n{args.num_models}_s{args.seed}"
            # load_name += f"_{args.train_mode}_n{args.num_models}"
            if args.lambda_1 > 0.:
                load_name += f"_la{args.lambda_1}"
            if args.lambda_2 > 0.:
                load_name += f"_lb{args.lambda_2}"
            model = []
            for i in range(args.num_models):
                if args.train_mode == 'hyper':
                    sub_model = PrimaryNetwork(depth=args.depth, z_dim=128)
                else:
    ...
    if args.attack_type in ['pgd', 'all', "rand"]:
    ...
    if args.attack_type in ['autoattack', 'all', "rand"]:
        atk = torchattack.AutoAttack(model, norm='Linf', eps=args.eps, version='standard',
                                     n_classes=args.num_classes, n_queries=200)
        print_srt = equipped_atk_func(model, autoattack_loader, atk, 'autoattack', print_srt)

    # black and other attack
    ...
    if args.attack_type in ['black_all', 'square']:
        ...
        print_srt = equipped_atk_func(model, autoattack_loader, atk, 'square', print_srt)
    ...
    if args.attack_type in ['black_all', 'di2fgsm']:
        ...
        print_srt = equipped_atk_func(model, autoattack_loader, atk, 'di2fgsm', print_srt)
